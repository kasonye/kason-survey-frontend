...
export default function Detail() {
    ...
    const deleteOption = (index) => e => {
        ...
        if (Object.values(params).length > 0) {
            handler.deleteQuestionDetail(questionDetail.options[index].optionId);
    ...
    const saveQuestion = e => {
        if (Object.values(params).length > 0) {
            handler.updateQuestionDetail(questionDetail)
        } else {
            questionDetail.sequence = questionList.length + 1
            handler.saveQuestionDetail(questionDetail)
        }
